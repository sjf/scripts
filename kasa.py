...
def main():
  """
    Setup kasa smart plugs on wifi, because the Kasa app straight doesnt work.
    Connect to TP-LINK wifi network. Host must have IP 192.168.0.100, but it
    should be assigned this by default.
  """
  if len(sys.argv) < 2:
    print("Usage: kasa.py network password")
    sys.exit(1)
  sp = SmartPowerStrip("192.168.0.1")
  sp.set_wifi_credentials(sys.argv[1],sys.argv[2])
  print("Done")
